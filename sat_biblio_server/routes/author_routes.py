...
@sat_biblio.route("/api/auteur/lire/<int:id_>", methods=["GET"])
def lire_auteur(id_):
...
# jsonify(False, message="Vous n'êtes pas connecté")
@sat_biblio.route("/api/auteur/supprimer/<int:id_>", methods=["GET"])
@validation_connexion_et_retour_defaut("email", redirect("/api"))
def supprimer_auteur(id_):
    id_author = id_
...
@sat_biblio.route("/api/auteur/modifier/<int:id_>", methods=["POST"])
@validation_connexion_et_retour_defaut("email", redirect("/api"))
def modifier_auteur(id_):
    data = request.get_json()
